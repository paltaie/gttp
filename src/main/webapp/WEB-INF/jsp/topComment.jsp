	...
	<body>
		<div class="container">
			...
			<div class="col-md-12">
				...
				<div class="asdf">
					<p>Thread: "<a target="_blank" href="http://redd.it/${thread.id}">${thread.title}</a>" <c:if test="${not empty thread.url}">(<a href="${thread.url}">link</a>)</c:if></p> <span class="reddit-ups">${thread.ups}</span>, <span class="reddit-downs">${thread.downs}</span> by <a href="http://reddit.com/u/${thread.author}">/u/${thread.author}</a></p>
				</div>
			</div>
